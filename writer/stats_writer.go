...
func (w *StatsWriter) handleStats(stats []model.StatsBucket) {
	...
	for _, p := range payloads {
		// Synchronously send the paylods one after the other
...
// buildPayloads returns a set of payloads to send out, each paylods
// guaranteed to have the number of stats buckets under the given
// maximum.
func (w *StatsWriter) buildPayloads(stats []model.StatsBucket, maxEntriesPerPayloads int) ([]*model.StatsPayload, int, int) {
	if len(stats) == 0 { // It could happen apparently
		return []*model.StatsPayload{}, 0, 0
	}

	// 1. Get an estimate of how many payloads we need, based on
	//    the total number of map entries (i.e: sum of number of
	//    items in the stats bucket's count map). Called N for the
	//    rest of the description.  NOTE: we use the number of
	//    items in the count map as the reference, but in reality,
	//    what take place are the distributions. We are guaranteed
	//    the number of entries in the count map is > than the
	//    number of entries in the distributions maps, so the
	//    algorithm is correct, but indeed this means we could do
	//    better.
	nbEntries := 0
	for _, s := range stats {
	...
	if maxEntriesPerPayloads <= 0 || nbEntries < maxEntriesPerPayloads { // Nothing to do, break early.
	...
	// 2. Create a slice of N maps, mapping a time window (stat +
	//    duration) to a stat bucket. We will build the payloads
	//    from these maps. This allows is to have one stat bucket
	//    per time window.
	...
	//    a same entry in several inputted stat buckets. We must
	//    check we never overwrite a entry in the new stats
	//    buckets but cleanly merge instead.
	i := 0
	for _, sb := range stats {
		tw := timeWindow{sb.Start, sb.Duration}

		for ekey, e := range sb.Counts {
			...
			if _, ok := sb.Distributions[ekey]; ok {
				if _, ok := newsb.Distributions[ekey]; ok {
					newsb.Distributions[ekey].Merge(sb.Distributions[ekey])
				} else {
					newsb.Distributions[ekey] = sb.Distributions[ekey]
				}
			}
			if _, ok := sb.ErrDistributions[ekey]; ok {
				if _, ok := newsb.ErrDistributions[ekey]; ok {
					newsb.ErrDistributions[ekey].Merge(sb.ErrDistributions[ekey])
				} else {
					newsb.ErrDistributions[ekey] = sb.ErrDistributions[ekey]
				}
			}
			i++
		}
	...
	// 4. Create the N payloads from the maps.
	nbStats := 0
	nbEntries = 0
