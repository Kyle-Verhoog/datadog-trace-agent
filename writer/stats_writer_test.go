...
func TestStatsWriter_BuildPayloads(t *testing.T) {
	...
	// Common case, no duplicate entries
	{
		...
		// Compute our expected counts by entries
		expectedCounts := make(map[string]float64, expectedNbEntries)
		for _, s := range stats {
			for ekey, e := range s.Counts {
				v, ok := expectedCounts[ekey]
				if !ok {
					v = 0
				}
				v += e.Value
				expectedCounts[ekey] = v
			}
		}
		...
		// 		for ekey, e := range s.Counts {
		// 			assert.Equal(expectedCounts[ekey], e.Value)
		// 		}
		// 	}
		// }
		// Assert the counts match
	...
	// Common case, with duplicate entries
	{
		sw, _, _, _ := testStatsWriter()
